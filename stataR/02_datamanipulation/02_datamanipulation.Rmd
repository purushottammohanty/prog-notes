...
The `dplyr` pipe operation implies that dataset is grouped as long as a separate `ungroup()` function is provided. It's a healthy practice to provide `ungroup()` function after the end of the grouped operation.
```{r, eval=F}
df %>% 
  group_by(continent, year) %>% # grouping variables
  mutate(mean_pop = mean(pop)) %>% # group wise operation to perform
